#ifndef SMP_ENGINE_ABS_F7_H
#define SMP_ENGINE_ABS_F7_H
#include "utils/stopwatch.h"
#include "utils/types.h"

class AbsSmpEngineF7 {
public:
  virtual ~AbsSmpEngineF7() = default;
  ...
  auto FindStableMatching() -> Matching {
    ...
    if (!is_perfect) {
      // Init Phase
      this->sw_.Start();
      this->InitProcOnDevice(); // Do the computation
      this->sw_.Stop();
  ...
  virtual void PrintProfilingInfo() {
    ...
    std::cout << this->GetEngineName() << " Total Time: " << this->precheck_time_ + this->init_time_ + this->core_time_
        << " ms" << std::endl;
    std::cout << "==================================================================" << std::endl;
    ...
    LOG(INFO) << this->GetEngineName() << " Total Time: " << this->precheck_time_ + this->init_time_ + this->core_time_
        << " ms";
    LOG(INFO) << "==================================================================";
  }
...
protected:
  ...
  virtual void InitProcOnDevice() = 0;
