...
namespace bamboosmp {
  SmpEngineLa2F8Top::SmpEngineLa2F8Top(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineLa2F8Top::InitProc() {
  ...
  void SmpEngineLa2F8Top::CoreProc() {
    const int n = this->n_;

    int block_size = 1024;
    int grid_size = (n + block_size - 1) / block_size;

    La2F8CASTopKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
      n,
      RawPtr(dev_prmtx_),
  ...
  auto SmpEngineLa2F8Top::PostProc() -> Matching {
