...
namespace bamboosmp {
  SmpEngineBamboo::SmpEngineBamboo(const SmpObj &smp, size_t n)
    : smp_(smp),
      ...
      host_prmtx_view_(host_prmtx_.data()),
      // Host-MetaData
  ...
  void SmpEngineBamboo::InitProc() {
    ...
    const int grid_size = (SIZE_MUL(n, n) + block_size - 1) / block_size;

    // Mw3F8CASBottomKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
    ...
    F9RankMatrixInitKernel<<<grid_size, block_size, 0, default_stream_.cuda_stream()>>>(
      n,
      pref_list_w_dview.data(),
      rank_mtx_w_dview.data()
    );
    ...
    F9PRMatrixInitKernel<<<grid_size, block_size, 0, default_stream_.cuda_stream()>>>(
      n,
      pref_list_m_dview.data(),
      rank_mtx_w_dview.data(),
      prmtx_dview.data()
    );
  ...
  void SmpEngineBamboo::DoWorkOnCpu() {
    ...
    if (this->unmatched_num_ == 1) {
      ...
      std::cout << "CPU starts LAProcedure." << std::endl;
  ...
  inline void SmpEngineBamboo::LAProcedure(int m) {
    ...
    while (!is_matched) {
      ...
      temp_node = this->host_prmtx_view_[IDX_MUL_ADD(m_idx, n_, w_rank)];
