...
namespace bamboosmp {
  SmpEngineGs2F9::SmpEngineGs2F9(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  SmpEngineGs2F9::~SmpEngineGs2F9() {
    delete[] atomic_partner_rank_;
  }

  void SmpEngineGs2F9::InitProc() {
  ...
  void SmpEngineGs2F9::InitQueues() {
  ...
  void SmpEngineGs2F9::CoreProc() {
    ...
    for (int tid = 0; tid < this->num_threads_; ++tid) {
      threads.emplace_back(&SmpEngineGs2F9::GsParTasklet, this, tid);
    }
  ...
  void SmpEngineGs2F9::GsParTasklet(int tid) {
  ...
  auto SmpEngineGs2F9::PostProc() -> Matching {
