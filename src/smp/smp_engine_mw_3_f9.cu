...
namespace bamboosmp {
  SmpEngineMw3F9::SmpEngineMw3F9(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineMw3F9::InitProc() {
    ...
    // H2D
    this->dev_rank_mtx_w_.SetToDevice(this->host_rank_mtx_w_);

    // Initialialze Metadata
    this->dev_partner_rank_.Fill(n_);
    this->dev_next_proposed_w_.Fill(0);
  }

  void SmpEngineMw3F9::CoreProc() {
    const int n = this->n_;
  ...
  auto SmpEngineMw3F9::PostProc() -> Matching {
