...
namespace bamboosmp {
  SmpEngineMw4F8TRight::SmpEngineMw4F8TRight(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineMw4F8TRight::InitProc() {
  ...
  void SmpEngineMw4F8TRight::CoreProc() {
    ...
    Mw4F8TopRightKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
      n,
      pref_list_m_dview,
      pref_list_w_dview,
      rank_mtx_w_dview,
  ...
  auto SmpEngineMw4F8TRight::PostProc() -> Matching {
