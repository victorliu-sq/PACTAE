...
namespace bamboosmp {
  SmpEngineMw4F8TLeft::SmpEngineMw4F8TLeft(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineMw4F8TLeft::InitProc() {
  ...
  void SmpEngineMw4F8TLeft::CoreProc() {
    // const int n = this->n_;
    const int n = this->n_;
    ...
    Mw4F8TopLeftKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
      n,
      pref_list_m_dview,
      pref_list_w_dview,
      rank_mtx_w_dview,
  ...
  auto SmpEngineMw4F8TLeft::PostProc() -> Matching {
