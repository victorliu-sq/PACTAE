...
namespace bamboosmp {
  SmpEngineLa3F8Bottom::SmpEngineLa3F8Bottom(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineLa3F8Bottom::InitProc() {
  ...
  void SmpEngineLa3F8Bottom::CoreProc() {
    const int n = this->n_;
    // auto prmtx_dview = this->dev_prmtx_.DeviceView();
    //
    // auto pref_list_w_dview = this->dev_pref_lists_w_.DeviceView();
  ...
  auto SmpEngineLa3F8Bottom::PostProc() -> Matching {
