...
namespace bamboosmp {
  SmpEngineMw3F8Top::SmpEngineMw3F8Top(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineMw3F8Top::InitProc() {
  ...
  void SmpEngineMw3F8Top::CoreProc() {
    const int n = this->n_;
    ...
    //     w_rank += 1;
    //
    //     // atomicCAS(&partner_rank_dview[w_idx], n, mi_rank);
    ...
    Mw3F8CASTopKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
      n,
      pref_list_m_dview,
      pref_list_w_dview,
      rank_mtx_w_dview,
  ...
  auto SmpEngineMw3F8Top::PostProc() -> Matching {
