...
namespace bamboosmp {
  SmpEngineLa3F8TLeft::SmpEngineLa3F8TLeft(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineLa3F8TLeft::InitProc() {
  ...
  void SmpEngineLa3F8TLeft::CoreProc() {
    const int n = this->n_;

    int block_size = 1024;
    int grid_size = (n + block_size - 1) / block_size;

    La3MinF8TopLeftKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
      n,
  ...
  auto SmpEngineLa3F8TLeft::PostProc() -> Matching {
