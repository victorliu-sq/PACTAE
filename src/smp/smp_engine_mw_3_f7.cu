...
namespace bamboosmp {
  SmpEngineMw3F7::SmpEngineMw3F7(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineMw3F7::InitProc() {
    ...
    this->ExecuteNTasklet(SIZE_MUL(n, n), [=] __device__(size_t tid) mutable {
      ...
      size_t row_index = IDX_MUL(w_idx, n);
  ...
  auto SmpEngineMw3F7::PostProc() -> Matching {
