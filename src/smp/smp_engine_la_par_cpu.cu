...
namespace bamboosmp {
  SmpEngineLaParCpu::SmpEngineLaParCpu(const SmpObj &smp, int size)
    : smp_(smp), n_(size) {
    cudaSetDevice(0);
  ...
  auto SmpEngineLaParCpu::GetStableMarriage() const -> std::vector<int> {
  ...
  void SmpEngineLaParCpu::PrintMatching() const {
  ...
  void SmpEngineLaParCpu::PostProc() {
    LOG(INFO) << "SmpEngineLaParCpu: PostProc Starts";
    for (int w = 0; w < n_; w++) {
      int m = pref_lists_w_view_[IDX_MUL_ADD(w, n_, atomic_partner_rank_[w].load())];
      stable_matching_[m] = w;
    }
    LOG(INFO) << "SmpEngineLaParCpu: PostProc Completes";
  }

  /* ------------------------------------- Init ------------------------------------------------ */
  /* ------------------------------------------------------------------------------------------------ */
  void SmpEngineLaParCpu::InitProc() {
  ...
  void SmpEngineLaParCpu::CoreProc() {
    ...
    //     threads.clear();
    //   }
    //   threads.emplace_back(&SmpEngineLaParCpu::LAParallelProcedure, this, tid);
    // }
    //
    ...
    const int max_num_threads = std::min(std::thread::hardware_concurrency(), 96u);
    const int men_per_thread = (n_ + max_num_threads - 1) / max_num_threads;

    std::vector<std::thread> threads;
    threads.reserve(max_num_threads);

    auto thread_worker = [this](int start_man, int end_man) {
      for (int m = start_man; m < end_man; ++m) {
        LAParallelProcedure(m);
      }
    };

    for (unsigned thread_id = 0; thread_id < max_num_threads; ++thread_id) {
      int start_man = thread_id * men_per_thread;
      int end_man = std::min(start_man + men_per_thread, n_);
      if (start_man >= end_man) {
        break; // Avoid launching extra threads
      }
      threads.emplace_back(thread_worker, start_man, end_man);
    }
    ...
    auto end_time = getNanoSecond();
    std::cout << "Exec Phase - LA-Par-CPU is : " << (end_time - start_time) / 1e6 << " ms" << std::endl;
  }

  void SmpEngineLaParCpu::LAProcedure(int m) {
  ...
  void SmpEngineLaParCpu::LAParallelProcedure(int m) {
    // auto host_partner_rank_ptr = host_partner_rank_.data();
    auto host_next_proposed_w_ptr = host_next_proposed_w_.data();
    int w_idx, m_rank, m_idx, w_rank, p_rank;
    m_idx = m;
