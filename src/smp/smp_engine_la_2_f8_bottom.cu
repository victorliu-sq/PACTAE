...
namespace bamboosmp {
  SmpEngineLa2F8Bottom::SmpEngineLa2F8Bottom(const SmpObj &smp, size_t n)
    : smp_(smp),
      n_(n),
  ...
  void SmpEngineLa2F8Bottom::InitProc() {
  ...
  void SmpEngineLa2F8Bottom::CoreProc() {
    ...
    La2F8CASBottomKernel<<<grid_size, block_size, 0, cuda_stream_.cuda_stream()>>>(
      n,
      RawPtr(dev_prmtx_),
  ...
  auto SmpEngineLa2F8Bottom::PostProc() -> Matching {
